...
def generatePrescription(retrievedDiseaseTuples, query):
	for diseaseTuple in retrievedDiseaseTuples:
		with open("prescription/" + str(retrievedDiseaseTuples.index(diseaseTuple)) + "_Prescription_" + diseaseTuple[0] + ".html", "w") as f:
			htmlLines = []
			...
			lines = sorted(lines, key=operator.itemgetter(2))[:10]
			#print(lines);
